...
void left_led_control_task(void);
void right_led_control_task(void);
...
EXPORT INT usermain(void) {
    ...
    // 左ウィンカーLED制御タスクの生成・実行
    ctsk.task = (FP)left_led_control_task;
    tk_cre_tsk(&ctsk);
    tk_sta_tsk(&ctsk, 0);

    // 右ウィンカーLED制御タスクの生成・実行
    ctsk.task = (FP)right_led_control_task;
    tk_cre_tsk(&ctsk);
    tk_sta_tsk(&ctsk, 0);

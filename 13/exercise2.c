...
// CoAPサーバのタスク
LOCAL void coap_task(void *exinf) {
    static   coap_init_t coap;
    ER err;
    ...
    while (1) {
        coap_accept(&req, TMO_FEVR);  // CoAP要求の受信
        ...
        coap_reply(&rpl, TMO_FEVR);  // CoAP応答の送信
    }
}

// ユーザメイン関数
EXPORT INT usermain(void) {
    T_CTSK drv_ctsk, coap_ctsk;   // タスク生成情報
    ER err;
    ...
    // CoAPサーバタスク生成・実行
    coap_ctsk.tskatr = TA_HLNG | TA_RNG3;  // タスク属性
    coap_ctsk.task = (FP)coap_task;        // CoAPサーバタスクの処理関数
    coap_ctsk.itskpri = 10;                // タスク起動時優先度
    coap_ctsk.stksz = 1024;                // スタックサイズ

    coap_tskid = tk_cre_tsk(&coap_ctsk);   // CoAPサーバタスクの生成
    tk_sta_tsk(coap_tskid, 0);             // CoAPサーバタスクの起動

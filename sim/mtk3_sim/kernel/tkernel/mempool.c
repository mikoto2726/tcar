...
#else
		end = (QUEUE*)((VB*)mplcb->mempool + mplcb->imacb.memsz);
#endif
	} else {
		end = aq->next;
	}
...
#else
	aq = ((void *)q == mplcb->mempool) ? &(mplcb->imacb.areaque) :  q - 1;
#endif
...
#else
	IMACB*	imacb = &(mplcb->imacb);
	aq = (blk == mplcb->mempool) ? &(mplcb->imacb.areaque) : (QUEUE*)blk - 1;
#endif
...
#else
		fq = (aq == &(mplcb->imacb.areaque)) ? (QUEUE*)(mplcb->mempool) : aq + 1;
#endif
...
#else
	if ( aq == &(mplcb->imacb.areaque) || aq->next == &(mplcb->areaque_end) ) {
#endif
...
#else
	QueInit(&mplcb->imacb.areaque);
	QueInit(&mplcb->imacb.freeque);

	/* Register onto AreaQue */
	knl_insertAreaQue(&mplcb->imacb.areaque, &mplcb->areaque_end);

	/* Set AREA_USE for locations that must not be free area */
	setAreaFlag(&mplcb->areaque_end, AREA_USE);

	/* Register onto FreeQue */
	knl_appendFreeAreaBound(mplcb, &mplcb->imacb.areaque);
#endif
...
#else
	if ( (B*)blk < (B*)mplcb->mempool || (B*)blk > (B*)mplcb->mempool + mplcb->imacb.memsz ) {
#endif
		ercd = E_PAR;
		goto error_exit;
	}
...
#else
		for ( fq = mplcb->imacb.freeque.next; fq != &mplcb->imacb.freeque; fq = fq->next ) {
#endif
			blksz = FreeSize(fq);
			frsz += blksz;
